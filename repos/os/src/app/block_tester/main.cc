...
struct Test::Main
{
	...
	Scratch_buffer _scratch_buffer { nullptr, 0 };
	...
	Main(Genode::Env &env) : _env(env)
	{
		...
		if (!_heap.alloc((size_t)_scratch_buffer_size,
		                 (void**)&_scratch_buffer.base)) {
			Genode::error("could not allocate scratch buffer with size ",
			              _scratch_buffer_size);
			throw Genode::Exception();
		}
		_scratch_buffer.size = _scratch_buffer_size;
